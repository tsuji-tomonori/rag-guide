...
TECHNIQUES: tuple[Technique, ...] = (
    ...
    technique("BM25", r"(?<![A-Za-z0-9])BM25(?!F)", "lexical retrieval", "candidate generation", "probabilistic sparse ranking", "https://doi.org/10.1561/1500000019", "語頻度飽和、逆文書頻度、文書長補正を用いる順位付け。", "lexical=true; sparse=true; tf_saturation=true; length_normalization=true", "bm25ContractTest"),
    ...
    technique("SPLADE", r"(?<![A-Za-z0-9])SPLADE(?![A-Za-z0-9]|\s*v?2)", "learned sparse retrieval", "candidate generation", "learned sparse vocabulary weights", "https://arxiv.org/abs/2107.05720", "語彙単位の疎な重みと展開を学習する第一段階検索器。", "learned=true; sparse=true; inverted_index_compatible=true", "spladeContractTest"),
    ...
    technique("ANCE", r"(?<![A-Za-z0-9])ANCE(?![A-Za-z0-9])", "dense retrieval", "training", "ANN-mined hard negatives", "https://arxiv.org/abs/2007.00808", "ANNインデックスから難しい負例を継続的に取得して学習する。", "dense=true; ann_negative_mining=true"),
    ...
    technique("HNSW", r"(?<![A-Za-z0-9])HNSW(?![A-Za-z0-9])|Hierarchical Navigable Small World", "ANN", "vector index", "hierarchical proximity graph", "https://arxiv.org/abs/1603.09320", "階層化した近傍グラフを探索する近似最近傍法。", "ann=true; graph_index=true; recall_latency_tradeoff=true", "hnswContractTest"),
    technique("Product Quantization", r"Product Quantization|直積量子化|\bPQ\b", "ANN", "vector compression", "subvector quantization codes", "https://doi.org/10.1109/TPAMI.2010.57", "ベクトルを部分空間へ分け、短い量子化コードで表す。", "ann_support=true; compressed_vectors=true"),
    ...
    technique("ColBERTv2", r"ColBERTv2", "late-interaction retrieval", "candidate generation", "compressed token-level multi-vectors", "https://arxiv.org/abs/2112.01488", "残差圧縮等でColBERTの索引量を抑える。", "dense=true; multi_vector=true; compressed_index=true"),
    ...
    technique("IRCoT", r"(?<![A-Za-z0-9])IRCoT(?![A-Za-z0-9])|Interleaved retrieval|交互検索", "iterative retrieval", "pre-retrieval", "interleaved retrieval and reasoning", "https://arxiv.org/abs/2212.10509", "複数段階質問で検索と推論を交互に進める。", "iterative=true; bounded_steps=true; evidence_trace=true"),
    ...
    technique("BERT passage reranking", r"BERT passage reranking|BERTによる文書断片の再順位付け|交差符号化型", "reranking", "post-retrieval", "cross-encoder", "https://arxiv.org/abs/1901.04085", "質問と候補を同時に符号化して関連度を再評価する。", "reads_candidate_text=true; candidate_generation=false; output_subset=true", "crossEncoderContractTest"),
