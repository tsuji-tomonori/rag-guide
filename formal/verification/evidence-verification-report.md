...
Issue #35向けに追加した2定理を含む425公開定理の`#print axioms`はCIで追加公理なしを確認しました。追加定理は678行の件数と、BLOCKEDが残る間に完了を報告しないことだけを対象とします。厳格監査では`sorry`、`admit`、`axiom`、`unsafe`、`native_decide`を禁止し、トレーサビリティは13要件へ更新しました。
