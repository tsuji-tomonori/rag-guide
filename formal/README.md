...
総合判定は **不合格（残る8指摘は修正後に再レビューが必要）** です。Critical 1件、Major 6件、Minor 1件は未解消ですが、Issue #35 Wave 4の独立監査・本文反映ゲートは完了しました。形式モデルそのものは、元の横断契約に加え、BM25等14件の具体技術契約の型検査、決定的シナリオ、5,000件のランダムトレース、Apalache境界付き検査を通過しました。Lean 4は、センテンス根拠台帳の件数、未カバー集合、一次資料割当、割合計算、40件の制御語彙atom導出、BM25・RRFの記号式性質に加え、469件の「文献要旨の形式命題ならば分解済みguide論理式」という条件付き導出を追加公理なしで検査します。
...
| 項目 | 値 |
|---|---:|
| 正本commit | `5324ee5f0aa630fc5d933470d9be63351fa703cf` |
| Markdownファイル | 99 |
| 説明状態 | 353 |
| 章序文 | 11 |
| 番号付き節 | 342 |
...
| 要旨・仕様ページを直接検査した一次資料 | 194/194（100%） |
| 明示的Abstractを直接検査した一次資料 | 128/194（65.98%） |
| 形式命題の主張極性をcuration済みの公式仕様 | 16/194（8.25%） |
...
| 制御語彙へ形式化できたセンテンス | 41/689（5.95%） |
| source atomsからLeanで導出したatom射影 | 40/689（5.81%） |
| Atom/AND/OR/IMPLIES/NOTへ論理分解できたセンテンス | 644/689（93.47%） |
| 一次資料候補を割り当てたセンテンス | 689/689（100%） |
| 文献要旨の形式命題からLeanで条件付き導出した論理式 | 469/689（68.07%） |
| 含意・否定の関係構造が未証明のセンテンス | 53/689（7.69%） |
| Issue #35最終判定・独立レビュー | 678/678（100%） |
| Issue #35本文反映 | 655/655（100%） |
| 自然言語全文のkernel-certified自動意味論証明 | 0/689（0%） |
| 経験的結果・実サービス動作の独立真偽確認 | 0/689（0%） |
...
各指摘の場所、根拠、影響、修正案、再検証方法は `formal/review-data/findings.csv` にあります。Wave 4では独立監査済み候補を67ファイルへ反映し、旧文と新文の対応をapplication mapで検証しました。この反映は8指摘そのものを解消したという意味ではありません。
...
- `formal/review-data/appropriateness_review_application_map.csv`: 書換え・限定・削除・矛盾対応の本文hashとtarget sentence対応
- `formal/review-data/appropriateness_relation_evidence.csv`: NOT・IMPLIES関係の資料根拠・仕様契約・書換え解消
- `formal/review-data/appropriateness_sheet_export.csv`: Google Sheetsへの決定的一方向export
...
- `formal/review-data/primary_sources.csv`: 195外部資料のURL、題名、著者・管理主体、年、利用節
- `formal/review-data/explanation_states.csv`: 353説明状態の追跡表
- `formal/review-data/findings.csv`: 不適切・曖昧・矛盾表現の指摘
- `formal/review-data/concrete_technologies.csv`: BM25等47具体技術の機構、一次資料、形式化対応
- `formal/review-data/trusted_primary_sources.csv`: 195資料の一次性・信頼tier・適格性
- `formal/review-data/sentence_evidence.csv`: 4,738センテンス相当行の根拠要否・資料対応
- `formal/review-data/uncovered_sentences.csv`: 一次資料が未対応の284センテンス
- `formal/review-data/coverage_summary.csv`: カバレッジ分子・分母・百分率
- `formal/review-data/semantic_assurance.csv`: 689文のatom形式化、導出、全文意味論、真偽ステータス
...
- `formal/review-data/literature_source_theorems.csv`: 194適格一次資料の検査状態、形式要旨命題、URL
- `formal/review-data/sentence_logical_proofs.csv`: 689文の論理分解、候補文献、未支持atom、Lean定理、保証境界
...
- `formal/lean/`: 生成台帳、505宣言、13要件のトレーサビリティ
...
Quintモデルは、認可、tenant、削除、来歴、時点、根拠十分性、矛盾、grounding、引用、tool実行、release、auditの横断契約と、具体技術の機構を有限状態へ抽象化したものです。Leanは、生成台帳の集合・件数・資料割当・割合、制御文法40件の`guide atoms ⊆ source atoms`、および469件の`source-summary facts → guide formula`を証明します。後者は文献要旨を真理公理として導入せず、任意のvaluationにおける明示的な条件付き含意として証明しています。Wave 4では対象632文の主張極性を二名でレビューしましたが、文献台帳側では論文等178資料の要旨射影について、著者自身の主張と比較対象・背景言及を区別する追加curationが残ります。さらに、日本語・英語からatom・論理式への変換が意味を完全保存することを検査するkernel-certified semantic parserはないため、自然言語全文の自動証明は0件です。論文の実験結果やホストされたサービスの実動作も一次資料の記述だけから真とせず、独立再現・runtime観測は0件として明示します。この境界は`formal/lean/traceability.json`で`INCONCLUSIVE`として追跡します。
