...
`decisions.csv` は手動台帳です。初回だけ `--initialize-decisions` で作成し、以後は通常実行で保持します。文面変更でsentence IDが変わった場合は、旧行を黙って流用せず、旧IDと新IDの対応を別途レビューします。
...
10. 一次レビューとは別の担当者が、一次資料・日本語意味・領域知識を再確認します。二名が同じ場合は最終判定になりません。
