...
assert.equal(audience.index(), 3);
assert(audience.document.getElementById('speaker').hidden);
const speaker = app('?session=one&presenter=1');
assert.equal(speaker.index(), 3);
assert.equal(speaker.document.getElementById('duration').value, '30');
audience.click('next'); assert.equal(audience.index(), 4); assert.equal(speaker.index(), 4);
speaker.click('next'); assert.equal(audience.index(), 5); assert.equal(speaker.index(), 5);
audience.key('ArrowRight', audience.document.getElementById('next')); assert.equal(audience.index(), 6);
...
audience.key('End'); assert.equal(audience.index(), 21); assert(audience.document.getElementById('next').disabled);
audience.click('next'); assert.equal(audience.index(), 21);
audience.key(' ', audience.document.body, { shiftKey: true }); assert.equal(audience.index(), 20);
audience.key('PageUp'); assert.equal(audience.index(), 19);
audience.key('PageDown'); assert.equal(audience.index(), 20);
audience.key('ArrowRight', audience.document.body, { ctrlKey: true }); assert.equal(audience.index(), 20);
audience.click('overview-open'); audience.key('Home'); assert.equal(audience.index(), 20);
const link = audience.document.querySelector('[data-jump="stage-layer-map"]');
const click = audience.document.createEvent('Event'); click.initEvent('click', true, true); link.dispatchEvent(click);
assert.equal(audience.index(), 5); assert.equal(speaker.index(), 5); assert(!audience.document.getElementById('overview').open);
assert.equal(audience.location().hash, '#stage-layer-map');
...
const other = app('?session=two'); other.click('next'); assert.equal(audience.index(), 5);
